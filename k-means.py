...
shirt_count = np.bincount(
    classification[:int(img.shape[0] * 0.6), int(img.shape[1] * 0.33):int(img.shape[1] * 0.66)].flatten())
pants_count = np.bincount(
    classification[int(img.shape[0] * 0.7):, int(img.shape[1] * 0.33):int(img.shape[1] * 0.66)].flatten())
